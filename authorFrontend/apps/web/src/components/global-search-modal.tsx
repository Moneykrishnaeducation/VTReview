...
export function GlobalSearchModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 md:pt-24 bg-black/50 dark:bg-black/80 backdrop-blur-xs p-4 animate-in fade-in duration-150">
      <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-2xl w-full flex flex-col shadow-2xl overflow-hidden animate-in slide-in-from-top-4 duration-200">
        {/* Search Input Box */}
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center gap-3 bg-slate-50 dark:bg-slate-900/50">
          ...
          <input
            ...
            autoFocus
            className="w-full bg-transparent border-none text-sm text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-0"
          />
          {query && (
          ...
          <button
            ...
            className="text-[11px] font-mono text-slate-600 dark:text-slate-400 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 px-2 py-1 rounded transition-colors"
          >
            ESC
          </button>
        </div>
        ...
        <div className="max-h-[60vh] overflow-y-auto p-3 space-y-4 text-xs">
          {!trimmed && (
            <div className="p-6 text-center text-slate-500 space-y-2">
              ...
              <div className="flex flex-wrap justify-center gap-2 pt-2">
                <span className="px-2 py-1 bg-slate-100 dark:bg-slate-900 rounded font-mono text-[10px] text-slate-600 dark:text-slate-400">"IC Markets"</span>
                <span className="px-2 py-1 bg-slate-100 dark:bg-slate-900 rounded font-mono text-[10px] text-slate-600 dark:text-slate-400">"ASIC"</span>
                <span className="px-2 py-1 bg-slate-100 dark:bg-slate-900 rounded font-mono text-[10px] text-slate-600 dark:text-slate-400">"EVD-2026-00481"</span>
                <span className="px-2 py-1 bg-slate-100 dark:bg-slate-900 rounded font-mono text-[10px] text-slate-600 dark:text-slate-400">"SimonB_FX"</span>
              </div>
            </div>
          )}
          ...
          {matchingBrokers.length > 0 && (
            <div>
              <div className="px-2 py-1 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
              ...
              <div className="space-y-1 mt-1">
                {matchingBrokers.map((b) => (
                  <button
                    ...
                    className="w-full text-left p-2.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-900 flex items-center justify-between group transition-colors cursor-pointer"
                  >
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <div className="font-bold text-slate-900 dark:text-slate-100 group-hover:text-amber-600 dark:group-hover:text-amber-400">{b.name}</div>
                        <div className="text-[11px] text-slate-500 dark:text-slate-400">{b.legalEntity} • {b.primaryRegulator}</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-amber-600 dark:text-amber-400 font-bold">{b.editorialScore} pts</span>
                      <ArrowRight className="h-4 w-4 text-slate-400 dark:text-slate-600 group-hover:text-slate-900 dark:group-hover:text-white transition-colors" />
                    </div>
                  </button>
                ))}
              </div>
          ...
          {matchingRegulators.length > 0 && (
            <div>
              <div className="px-2 py-1 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                <Scale className="h-3 w-3 text-blue-500 dark:text-blue-400" />
                ...
                {matchingRegulators.map((r) => (
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <div className="font-bold text-slate-900 dark:text-slate-100 group-hover:text-blue-600 dark:group-hover:text-blue-400">{r.code} — {r.name}</div>
                        <div className="text-[11px] text-slate-500 dark:text-slate-400">{r.jurisdiction} • {r.tier}</div>
                      </div>
                    </div>
                    <ArrowRight className="h-4 w-4 text-slate-400 dark:text-slate-600 group-hover:text-slate-900 dark:group-hover:text-white transition-colors" />
                  </button>
            ...
            <div>
              ...
              <div className="space-y-1 mt-1">
                {matchingEvidence.map((ev) => (
                  ...
                  >
                    <div>
                      <div className="font-mono text-emerald-600 dark:text-emerald-400 font-bold">{ev.id}</div>
                      <div className="text-slate-800 dark:text-slate-200 group-hover:text-slate-950 dark:group-hover:text-white">{ev.title}</div>
          ...
          {matchingGuides.length > 0 && (
            <div>
              <div className="px-2 py-1 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                <BookOpen className="h-3 w-3 text-indigo-500 dark:text-indigo-400" />
              ...
              <div className="space-y-1 mt-1">
                {matchingGuides.map((g) => (
                  ...
                  >
                    <div>
                      <div className="font-bold text-slate-900 dark:text-slate-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400">{g.title}</div>
