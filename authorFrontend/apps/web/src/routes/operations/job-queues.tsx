      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-xs">
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-800/80">
          {jobs.map((job) => (
            <div
              key={job.id}
              className="p-4 flex flex-wrap items-center justify-between gap-4 hover:bg-slate-50/80 dark:hover:bg-slate-850/40 transition-colors"
            >
              <div className="space-y-1 max-w-xl">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-amber-700 dark:text-amber-400 font-bold">{job.id}</span>
                  <span className="font-bold text-slate-900 dark:text-white text-sm">{job.jobName}</span>
              ...
              <div className="flex items-center gap-6 font-mono text-[11px]">
                <div className="text-right">
                  ...
                  <div className="text-slate-700 dark:text-slate-300">{job.attempts} / {job.maxAttempts}</div>
                </div>

                <div className="text-right">
                  <div className="text-slate-500 uppercase text-[10px] font-bold">Processed</div>
                  <div className="text-emerald-600 dark:text-emerald-400 font-bold">{job.recordsProcessed?.toLocaleString() || "—"} recs</div>
                </div>

                {job.status === "failed" && (
                  <button
                    onClick={() => handleRetry(job.id)}
                    className="px-3 py-1.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold rounded-lg flex items-center gap-1 cursor-pointer shadow-xs"
                  >
