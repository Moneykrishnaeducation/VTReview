  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pb-2 border-b border-slate-800">
        <Link to="/brokers" className="inline-flex items-center gap-1.5 text-xs text-slate-400 hover:text-white transition-colors">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center bg-slate-900 border border-slate-800 p-0.5 rounded-lg text-xs">
            <button
              ...
              className={`px-3 py-1 rounded-md font-semibold transition-colors cursor-pointer ${
                !isDraftMode ? "bg-amber-500 text-slate-950 shadow-xs" : "text-slate-400 hover:text-white"
              }`}
            >
              Public State
            </button>
          ...
          <button
            onClick={handleSave}
            className="px-4 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
          >
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 text-xs shadow-xs space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2.5 mb-1">
                <h1 className="text-xl md:text-2xl font-black text-white">{broker.name}</h1>
          ...
          <div className="flex items-center gap-3 bg-slate-950 p-3 rounded-xl border border-slate-800 font-mono">
            <div className="text-center px-2 border-r border-slate-800">
              ...
              <div className="text-base font-black text-amber-400">★ {broker.editorialScore.toFixed(1)}</div>
            </div>
            <div className="text-center px-2">
              <div className="text-[10px] text-slate-500 font-bold uppercase">Community</div>
              <div className="text-base font-black text-slate-200">★ {broker.communityScore.toFixed(1)}</div>
            </div>
          </div>
        </div>

        {/* 15-Tab Horizontal Scroller */}
        <div className="flex items-center gap-1 overflow-x-auto border-t border-slate-800 pt-3 custom-scrollbar">
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                ...
                <span>{tab.label}</span>
                {tab.badge !== undefined && (
                  <span className="text-[10px] font-mono px-1.5 py-0.2 rounded-full bg-slate-800 text-slate-300 font-bold">
                    {tab.badge}
                  </span>
                )}
              </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs">
          <div className="md:col-span-2 space-y-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
              <h3 className="font-bold text-white text-sm">Trading Benchmarks & Account Thresholds</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <div className="p-3 bg-slate-950 rounded-xl border border-slate-800">
                  <div className="text-slate-500 text-[10px] uppercase font-bold">EUR/USD Median Spread</div>
                  <div className="font-mono text-sm font-black text-emerald-400 mt-0.5">{broker.eurUsdSpread} pips</div>
                </div>
                <div className="p-3 bg-slate-950 rounded-xl border border-slate-800">
                  <div className="text-slate-500 text-[10px] uppercase font-bold">Round-Turn Commission</div>
                  <div className="font-mono text-sm font-black text-slate-200 mt-0.5">${broker.commissionPerLot * 2}/lot</div>
                </div>
                <div className="p-3 bg-slate-950 rounded-xl border border-slate-800">
                  <div className="text-slate-500 text-[10px] uppercase font-bold">Min Deposit Barrier</div>
                  <div className="font-mono text-sm font-black text-slate-200 mt-0.5">${broker.minDeposit}</div>
                </div>
                <div className="p-3 bg-slate-950 rounded-xl border border-slate-800">
                  <div className="text-slate-500 text-[10px] uppercase font-bold">Execution Engine</div>
                  <div className="font-mono text-sm font-black text-amber-400 mt-0.5">{broker.executionModel}</div>
                </div>
                <div className="p-3 bg-slate-950 rounded-xl border border-slate-800">
                  <div className="text-slate-500 text-[10px] uppercase font-bold">Max Leverage Ratio</div>
                  <div className="font-mono text-sm font-black text-slate-200 mt-0.5">{broker.maxLeverage.split("/")[0]}</div>
                </div>
                <div className="p-3 bg-slate-950 rounded-xl border border-slate-800">
                  <div className="text-slate-500 text-[10px] uppercase font-bold">Tradable Instruments</div>
                  <div className="font-mono text-sm font-black text-slate-200 mt-0.5">{broker.tradableAssetsCount}+ Assets</div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-3">
              <h3 className="font-bold text-white text-sm">Editorial Research Summary</h3>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
              ...
              <div className="space-y-2.5">
                {broker.licenses.map((lic) => (
                  <div key={lic.id} className="p-3 bg-slate-950 rounded-xl border border-slate-800 space-y-1.5">
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-slate-200">{lic.regulatorCode} ({lic.jurisdiction})</span>
                      <StatusBadge status={lic.tier} size="sm" />
                    </div>
                    <div className="font-mono text-[11px] text-slate-400">License #{lic.licenseNumber}</div>
      ...
      {activeTab === "regulation" && (
        <div className="space-y-6 text-xs">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
            <h3 className="text-base font-bold text-white">Direct Regulatory Register Certifications</h3>

            <div className="space-y-4">
              {broker.licenses.map((lic) => (
                <div key={lic.id} className="p-4 bg-slate-950 rounded-xl border border-slate-800 space-y-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-slate-100 text-sm">{lic.regulatorName}</span>
                      <span className="font-mono text-amber-400 font-bold">({lic.regulatorCode})</span>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 font-mono text-[11px] text-slate-400 pt-2 border-t border-slate-800/80">
                    <div>License Number: <strong className="text-slate-200">{lic.licenseNumber}</strong></div>
                    <div>Entity: <strong className="text-slate-200">{lic.licenseeEntity}</strong></div>
                    <div>Compensation: <strong className="text-slate-200">{lic.compensationScheme || "None"}</strong></div>
                    <div>NBP Protection: <strong className="text-emerald-400">{lic.negativeBalanceProtection ? "Guaranteed" : "No"}</strong></div>
                  </div>
                </div>
              ))}
            </div>
      ...
      {activeTab === "evidence" && (
        <div className="space-y-4 text-xs">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-bold text-white">Linked Evidence Vault Documents ({brokerEvidence.length})</h3>
            <Link to="/evidence" className="text-amber-400 hover:underline font-semibold">
      ...
      {activeTab === "audit_history" && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 text-xs space-y-4">
          <h3 className="text-base font-bold text-white">Immutable Audit Log for {broker.name}</h3>
      ...
      {!["overview", "regulation", "ratings", "evidence", "audit_history"].includes(activeTab) && (
        <div className="p-12 text-center bg-slate-900 border border-slate-800 rounded-2xl text-slate-400 space-y-2 text-xs">
