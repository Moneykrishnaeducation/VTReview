...
export default function RegulatorsList() {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
              <span>{row.code}</span>
              <span className="text-slate-500 dark:text-slate-400 font-normal">({row.name})</span>
            </div>
            <div className="text-[11px] text-slate-500 dark:text-slate-400">{row.jurisdiction}</div>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="text-[11px] text-slate-700 dark:text-slate-300">
          ...
          <div className="font-mono text-emerald-600 dark:text-emerald-400 text-[10px] font-bold">{row.maxCoverageFormatted}</div>
        </div>
      ),
    },
    {
      ...
      sortable: true,
      cell: (row) => <span className="font-mono font-bold text-amber-600 dark:text-amber-400">{row.activeBrokersCount} Verified</span>,
    },
    {
      header: "Last Audit",
    ...
    },
    {
      ...
      cell: (row) => (
        <a
          ...
          className="inline-flex items-center gap-1 text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300 hover:underline text-xs font-semibold"
        >
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 px-2.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-900 text-slate-600 dark:text-slate-400 font-mono text-[11px] mb-1">
            <Scale className="h-3.5 w-3.5 text-blue-500 dark:text-blue-400" />
      ...
      {selectedRegulator && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 text-xs space-y-4 animate-in fade-in shadow-xs">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-slate-500 dark:text-slate-400">{selectedRegulator.jurisdiction} • {selectedRegulator.tier}</div>
              </div>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 space-y-1">
              <div className="font-bold text-slate-900 dark:text-slate-200">Statutory Compensation Protection</div>
              <div className="text-slate-600 dark:text-slate-400">{selectedRegulator.compensationScheme}</div>
              <div className="font-mono text-emerald-600 dark:text-emerald-400 font-bold">{selectedRegulator.maxCoverageFormatted}</div>
            </div>

            <div className="p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 space-y-1">
              <div className="font-bold text-slate-900 dark:text-slate-200">Statutory Enforcement Powers</div>
