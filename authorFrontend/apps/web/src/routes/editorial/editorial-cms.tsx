  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 px-2.5 py-0.5 rounded-full bg-indigo-950/60 border border-indigo-800 text-indigo-300 font-mono text-[11px] mb-1">
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <span className="font-mono text-[10px] text-indigo-400 font-bold bg-indigo-950/60 border border-indigo-800 px-1.5 py-0.2 rounded">
                      {item.category}
                    </span>
        ...
        {selectedGuide ? (
          <div className="lg:col-span-7 bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-6">
            <div className="flex flex-wrap items-start justify-between gap-3 pb-4 border-b border-slate-800">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="font-mono text-xs text-indigo-400 font-bold bg-indigo-950/60 border border-indigo-800 px-2 py-0.5 rounded">
                    {selectedGuide.category}
                  </span>
                  <StatusBadge status={selectedGuide.status} size="sm" />
                </div>
                <h3 className="text-base font-bold text-white">{selectedGuide.title}</h3>
                <div className="text-slate-400 text-[11px]">
                  Author: <strong className="text-slate-200">{selectedGuide.authorName}</strong> ({selectedGuide.authorRole})
                </div>
              </div>
            </div>
            ...
            <div className="p-4 bg-slate-950 rounded-xl border border-slate-800 space-y-3">
              ...
              <div className="pt-2 border-t border-slate-800 flex flex-wrap items-center justify-between text-[11px] text-slate-500 font-mono">
                <div>Fact Checked: <strong className="text-slate-300">{selectedGuide.factCheckedBy || "Elena Rostova"}</strong></div>
                <div>Compliance Signed: <strong className="text-slate-300">{selectedGuide.complianceSignedBy || "Marcus Vance"}</strong></div>
              </div>
            </div>
            ...
            <div className="p-4 bg-slate-950 rounded-xl border border-slate-800 space-y-2">
              ...
              <p className="text-slate-300 leading-relaxed">{selectedGuide.summary}</p>
            </div>
            ...
            <div className="p-4 bg-slate-950 rounded-xl border border-slate-800 space-y-2 font-mono text-[11px]">
              ...
              <div>Slug: <span className="text-amber-400">/guides/{selectedGuide.slug}</span></div>
              <div className="text-slate-400 truncate">Title: {selectedGuide.seoTitle}</div>
            </div>

            {/* Publishing Action Bar */}
            <div className="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-slate-800">
              <div className="text-[11px] text-slate-500">
                Action by: <strong className="text-slate-300">{activeRoleDef.name}</strong>
              </div>
