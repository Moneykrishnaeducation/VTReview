  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 px-2.5 py-0.5 rounded-full bg-amber-950/60 border border-amber-800 text-amber-300 font-mono text-[11px] mb-1">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 text-xs space-y-4 shadow-xs">
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="font-bold text-white text-sm">{prop.brokerName}</span>
                  <span className="text-slate-400 font-medium">({prop.pillarName})</span>
                ...
                <div className="flex items-center gap-2 font-mono">
                  ...
                  <span className="text-slate-300 font-bold">{prop.currentTotalRating.toFixed(1)}</span>
                  <ArrowRight className="h-3 w-3 text-slate-500" />
                  <span className="text-amber-400 font-black">{prop.proposedTotalRating.toFixed(1)} pts</span>
                  <span
                    className={`px-1.5 py-0.2 rounded text-[10px] font-bold ${
                      prop.scoreDelta > 0 ? "bg-emerald-950 text-emerald-300" : "bg-rose-950 text-rose-300"
                    }`}
                  >
              ...
              <div className="p-3 bg-slate-900 rounded-lg border border-slate-800/80 text-slate-300 leading-relaxed">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pt-2 text-[11px] text-slate-500">
                <div>
                  Proposed by <strong className="text-slate-300">{prop.proposedBy}</strong> on {prop.proposedAt} • Linked Evidence: <span className="font-mono text-amber-400">{prop.evidenceId}</span>
                </div>
                ...
                {prop.status === "pending_review" && (
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-3 py-1.5 bg-rose-950 hover:bg-rose-900 text-rose-300 font-bold rounded-lg border border-rose-800 flex items-center gap-1 cursor-pointer"
                    >
                    ...
                    <button
                      ...
                      className="px-4 py-1.5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold rounded-lg flex items-center gap-1 cursor-pointer shadow-xs"
                    >
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-base font-bold text-white">Live Broker Rating Calculation Studio</h3>

          <select
            value={selectedBrokerId}
            onChange={(e) => setSelectedBrokerId(e.target.value)}
            className="bg-slate-900 border border-slate-800 rounded-lg px-3 py-1.5 text-xs text-slate-200 font-bold focus:ring-1 focus:ring-amber-500"
          >
            {brokers.map((b) => (
