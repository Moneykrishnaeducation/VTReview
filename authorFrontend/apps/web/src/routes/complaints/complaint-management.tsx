  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 px-2.5 py-0.5 rounded-full bg-rose-500/10 border border-rose-500/20 text-rose-700 dark:text-rose-300 font-mono text-[11px] mb-1">
              ...
              return (
                <div
                  ...
                  className={`p-3.5 rounded-xl border transition-all cursor-pointer space-y-2 ${
                    isSelected
                      ? "bg-rose-50/50 dark:bg-slate-950 border-rose-500/50 shadow-sm ring-1 ring-rose-500/20"
                      : "bg-slate-50 dark:bg-slate-950/60 border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700"
                  }`}
                >
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <span className="font-mono font-bold text-amber-700 dark:text-amber-400 text-[11px]">{c.caseNumber}</span>
                      <div className="font-bold text-slate-900 dark:text-white text-sm">{c.brokerName}</div>
                    </div>
                    <div className="text-right">
                      <div className="font-mono font-bold text-rose-600 dark:text-rose-400">${c.claimAmount.toLocaleString()}</div>
                      <StatusBadge status={c.status} size="sm" />
                    </div>
                  </div>

                  <div className="text-slate-600 dark:text-slate-400 text-[11px] line-clamp-1">{c.claimTitle}</div>
        ...
        {selectedCase ? (
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 space-y-6 shadow-xs">
            <div className="flex flex-wrap items-start justify-between gap-3 pb-4 border-b border-slate-100 dark:border-slate-800">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="font-mono text-xs text-amber-700 dark:text-amber-400 font-bold bg-amber-500/10 border border-amber-500/20 px-2 py-0.5 rounded">
                    {selectedCase.caseNumber}
                  </span>
                  <StatusBadge status={selectedCase.status} size="sm" />
                </div>
                <h3 className="text-base font-bold text-slate-900 dark:text-white">{selectedCase.claimTitle}</h3>
                <div className="text-slate-600 dark:text-slate-400 text-[11px]">
                  Claimant: <strong className="text-slate-800 dark:text-slate-200">{selectedCase.userName}</strong> ({selectedCase.userEmail}) vs <strong className="text-amber-600 dark:text-amber-400">{selectedCase.brokerName}</strong>
                </div>
              </div>

              <div className="text-right bg-slate-50 dark:bg-slate-950 p-3 rounded-xl border border-slate-200 dark:border-slate-800 font-mono">
                <div className="text-[10px] text-slate-500 uppercase font-bold">Disputed Amount</div>
                <div className="text-base font-black text-rose-600 dark:text-rose-400">${selectedCase.claimAmount.toLocaleString()} {selectedCase.currency}</div>
              </div>
            </div>
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 space-y-2">
              ...
              <p className="text-slate-700 dark:text-slate-300 leading-relaxed">{selectedCase.claimDescription}</p>
            </div>
            ...
            {linkedEvidence && (
              <div className="p-3.5 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className="font-bold text-slate-800 dark:text-slate-200">{linkedEvidence.title}</span>
            ...
            <div className="space-y-3">
              <h4 className="font-bold text-slate-900 dark:text-white text-xs">Dispute Case Timeline & Actions</h4>
              <div className="space-y-2">
                {selectedCase.timeline.map((t) => (
                  <div key={t.id} className="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800/80 space-y-1">
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-950/80 rounded-xl border border-slate-200 dark:border-slate-800 space-y-4">
              <h4 className="font-bold text-slate-900 dark:text-white text-xs">Mediation Action Console</h4>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 rounded-lg p-2.5 text-xs text-slate-900 dark:text-slate-200 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:ring-1 focus:ring-amber-500"
                />
              </div>
              ...
              <div className="flex flex-wrap items-center justify-end gap-2 pt-2">
                <button
                  ...
                  className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 font-bold rounded-lg border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer"
                >
                ...
                <button
                  ...
                  className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 dark:bg-emerald-500 dark:hover:bg-emerald-400 text-white dark:text-slate-950 font-bold rounded-lg flex items-center gap-1.5 transition-colors cursor-pointer shadow-xs"
                >
                  <CheckCircle2 className="h-4 w-4" />
