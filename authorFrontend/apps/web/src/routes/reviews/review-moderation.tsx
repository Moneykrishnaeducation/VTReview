            ...
            >
              ...
              <div className="flex flex-wrap items-start justify-between gap-3 pb-3 border-b border-slate-800">
                <div className="flex items-center gap-3">
                  <div className="h-9 w-9 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center font-bold text-slate-200 text-xs">
                    {rev.userAvatar}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {rev.isVerifiedTrader && (
                        <span className="inline-flex items-center gap-1 text-[10px] font-mono font-bold bg-emerald-950 text-emerald-300 border border-emerald-800 px-1.5 py-0.2 rounded">
                    ...
                    <div className="text-[11px] text-slate-400">
                      Target Broker: <strong className="text-amber-400">{rev.brokerName}</strong> • Submitted {rev.submittedAt}
                    </div>
                  </div>
                </div>

                <div className="flex items-center gap-1 text-amber-400 font-mono font-bold text-sm bg-slate-950 px-2.5 py-1 rounded-lg border border-slate-800">
              ...
              <div className="space-y-2">
                <h4 className="font-bold text-slate-100 text-sm">{rev.title}</h4>
              ...
              {rev.evidenceAttached && linkedEv && (
                <div className="p-3.5 bg-slate-950 rounded-xl border border-slate-800 flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="font-bold text-slate-200">{linkedEv.title}</span>
              ...
              {rev.brokerResponse && (
                <div className="p-3.5 bg-slate-950/70 border border-slate-800/80 rounded-xl space-y-1.5 pl-4 border-l-2 border-l-amber-500">
                  <div className="flex items-center gap-2 text-slate-400">
                    <CornerDownRight className="h-3.5 w-3.5 text-amber-400" />
                    <strong className="text-slate-200">{rev.brokerResponse.author}</strong> ({rev.brokerResponse.officialRole})
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-slate-800">
                <div className="text-[11px] text-slate-500">
                  Assigned Moderator: <strong className="text-slate-300">{rev.assignedModerator || "Unassigned"}</strong>
                </div>
